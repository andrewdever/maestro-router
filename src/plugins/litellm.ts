/**
 * @maestro/router — LiteLLM plugin.
 *
 * Universal LLM gateway supporting 2500+ models from 100+ providers
 * via an OpenAI-compatible interface. Can operate in two modes:
 *
 * - **Proxy mode**: Connects to a running LiteLLM proxy server.
 *   Set LITELLM_API_KEY and optionally base_url.
 * - **SDK mode**: Uses LiteLLM as an in-process SDK.
 *   Set mode='sdk' in config. (Requires Python runtime.)
 *
 ...
 * (apiKey, baseUrl, mode). Because ES modules are cached after first import,
...
const LITELLM_MODELS: ModelCapability[] = [
  {
    provider: 'anthropic',
...
const EFFORT_PREFERENCES: Record<Effort, string[]> = {
  deep: ['claude-opus-4-6', 'gpt-5', 'gemini-2.5-pro', 'deepseek-r1'],
  standard: ['claude-sonnet-4-6', 'gemini-2.5-pro', 'gpt-5', 'mistral-large'],
  minimal: ['claude-haiku-4-5', 'gpt-4.1-mini', 'gemini-2.5-flash', 'deepseek-r1'],
};

// ── Plugin State ──────────────────────────────────────────────────

/** API key for LiteLLM proxy. Set via initialize(). */
let apiKey: string | null = null;

/** Base URL for LiteLLM proxy. Defaults to localhost. */
let baseUrl = 'http://localhost:4000';

/** Operating mode: 'proxy' (default) or 'sdk'. */
let mode: 'proxy' | 'sdk' = 'proxy';

/** Cached model list with TTL. */
let modelCache: ModelCapability[] = [...LITELLM_MODELS];
...
function buildSelection(model: ModelCapability, intent: SpawnIntent): ModelSelection {
  return {
    ...
    rationale:
      `LiteLLM selection (${mode}): ${model.provider}/${model.model} ` +
...
/**
 * LiteLLM — universal gateway with 2500+ models, 100+ providers.
 * OpenAI-compatible interface. Supports proxy and SDK modes.
 *
 * Uses undici for live model discovery from the LiteLLM proxy,
...
export default {
  ...
  async initialize(config: Record<string, unknown>): Promise<void> {
    ...
    } else if (typeof config.LITELLM_API_KEY === 'string' && config.LITELLM_API_KEY.length > 0) {
    ...
    if (config.mode === 'sdk' || config.mode === 'proxy') {
      mode = config.mode;
    }
  },
  ...
  async select(intent: SpawnIntent): Promise<ModelSelection> {
    if (!apiKey && mode === 'proxy') {
      throw new SelectionError('litellm', 'LITELLM_API_KEY not configured for proxy mode');
    }

    // 1. Filter by capabilities
    let candidates = LITELLM_MODELS.filter(m => {
    ...
    if (candidates.length === 0) {
      throw new NoModelAvailableError(intent, 'No LiteLLM model matches requirements');
    }
  ...
  async models(): Promise<ModelCapability[]> {
    ...
    if (mode === 'sdk' || !apiKey) return [...LITELLM_MODELS];
    ...
    modelFetchPromise = (async () => {
      try {
        const response = await fetchJson<LiteLLMModelsResponse>(
          `${baseUrl}/model/info`,
  ...
  async healthy(): Promise<boolean> {
    if (mode === 'sdk') {
      // SDK mode: healthy as long as we have model data
      return true;
    }
    // Proxy mode: need API key
